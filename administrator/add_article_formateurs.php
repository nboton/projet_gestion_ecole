...
<div class="container mainbg">
      ...
      <br><a class="return" href="teachers_Room.php"><i class="glyphicon glyphicon-arrow-left"></i> <?php echo $lang ['return']; ?></a>
...
if (isset($_POST['submit'])) {
  ...
  if ((($_FILES["topic_img"]["type"] == "image/gif")
    ...
    && ($_FILES["topic_img"]["size"] < $size ) && in_array($extension, $allowedExts)) {
      ...
      if ($_FILES["topic_img"]["error"] > 0) {
        ...
        $stmt_article = $connect->prepare("INSERT INTO topics_formateurs (title, topic, image, date) VALUES (:title, :topic, '$topic_img', '$article_date')");
    ...
    $stmt_article = $connect->prepare("INSERT INTO topics_formateurs (title, topic, date) VALUES (:title, :topic, '$article_date')");
    $stmt_article->bindParam (':title' , $article_title , PDO::PARAM_STR );
    $stmt_article->bindParam (':topic' , $article_content , PDO::PARAM_STR );
    $stmt_article->execute();
}
  ...
  if (isset($stmt_article)) {
    header ("location: add_article_formateurs.php?article=success");   
    echo "<meta http-equiv='refresh' content='0; url = add_article_formateurs.php?article=error' />";
...
if (isset($_GET['article']) == "success") {
  echo "<meta http-equiv='refresh' content='2; url = teachers_Room.php' />";
