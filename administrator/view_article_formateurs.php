...
<?php 
  if (isset($_POST['delete_topic'])) {
      ...
      $stmt_delete = $connect->prepare("DELETE FROM topics_formateurs WHERE id=:id");
      ...
      if (isset($stmt_delete)) { 
        header ("location: teachers_Room.php");
        echo "<meta http-equiv='refresh' content='0; url = teachers_Room.php' />";
      }
...
if (isset($_GET['comment_delete']) && isset($_GET['token']) ) {
  ...
  if (isset($_SESSION ['admin_index'])) {
    ...
    if ($_GET['token'] == $_SESSION['token']) {
      ...
      $stmt_delete = $connect->prepare("DELETE FROM comments_formateurs WHERE id=:id AND article_id=:article_id");
      $stmt_delete->bindParam (':id' , $delete_id , PDO::PARAM_STR );
      $stmt_delete->bindParam (':article_id' , $get_id , PDO::PARAM_STR );
      $stmt_delete->execute();

      if (isset($stmt_delete)) {
        echo "<meta http-equiv='refresh' content='0; url = view_article_formateurs.php?id=".$get_id."' />";
      }
...
<div class="media">
      ...
      <div class="media-body">
        ...
        <a href="view_article_formateurs.php?id=<?php echo $get_id; ?>&comment_delete=<?php echo $comment_id; ?>&token=<?php echo $_SESSION ['token']; ?>" class="btn btn-danger"><?php echo $lang ['delete']; ?></a>  <span class="label label-default"><?php echo $comment_date; ?></span> 
      </div>
...
if (isset($_POST['submit'])) {
  ...
  $author = $_SESSION ['admin_index'];
   ...
   $stmt_comment = $connect->prepare("INSERT INTO comments_formateurs (comment, type, author, article_id, comment_date) VALUES (:comment, 'admin', '$author', :article_id, '$comment_date')");
